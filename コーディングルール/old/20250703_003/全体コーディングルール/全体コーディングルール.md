...
- data:
    ...
    tool_name: update_a_cell
    tool_label: update_a_cell
    desc: "Google Sheetsのセルを更新"
    tool_parameters:
      sheet: variable_selector: [llm, sheet_name]
      tab: variable_selector: [llm, tab_name]
      cell: variable_selector: [llm, cell_address]
      value: variable_selector: [llm, cell_value]
    tool_configurations:
      is_team_authorization: 1
...
- data:
    ...
    tool_parameters:
      sheet: 
        variable_selector: [start, sheet_name]
      tab: 
        variable_selector: [start, tab_name]
      cell: 
        variable_selector: [code_1, cell_address]
      value: 
        variable_selector: [code_1, cell_value]
    tool_configurations:
      is_team_authorization: 1
      spreadsheet_id: "1ABC...XYZ"  # 実際のスプレッドシートID
```
...
- data:
    ...
    provider_type: marketplace
    provider_name: "@langgenius/google-sheets"
    tool_name: read_a_range
    tool_label: read_a_range
    desc: "指定した範囲のデータを読み取り"
    tool_parameters:
      sheet:
        variable_selector: [start, sheet_name]
      tab:
        variable_selector: [start, tab_name]
      range:
        variable_selector: [code_1, range_address]
    tool_configurations:
      is_team_authorization: 1
