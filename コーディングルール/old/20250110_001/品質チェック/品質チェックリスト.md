...
## 🚨 超重要：最頻出エラーチェック（これだけで90%のエラーを防げる）

開発前・開発後に必ず確認してください：

### IF-ELSEノード
- [ ] **すべてのケースに`logical_operator: "and"`または`"or"`が設定されている（条件が1つでも必須！）**
- [ ] **比較演算子は`"not empty"`（`"is not empty"`ではない！）**
- [ ] **`id`と`case_id`が完全一致している**
- [ ] **case_idに'true'、'false'等の予約語的な値を使用していない**

### template-transformノード（Jinja2）
- [ ] **すべてのフィルターで`|`の後にスペースがない（`|trim`はOK、`| trim`はNG）**
- [ ] **template内で`{{#変数#}}`を使用していない（`{{ 変数 }}`を使用）**
...
### Vision機能（画像認識）
- [ ] **`vision.enabled: true`の場合、`vision.variable_selector`が正しく設定されている（空配列[]ではない）**

### その他の必須確認
- [ ] **YAMLファイル内にコメント（#で始まる行）が一切含まれていない**
- [ ] **すべてのケースに`case_id`が設定されている（`id`と同じ値）**
...
- [ ] **Jinja2フィルターのパイプ記号（|）とフィルター名の間にスペースがない**
  - 検索パターン：`| \w`（パイプの後にスペースがある箇所）
  - 例：`| join`、`| tojson`、`| trim`などはすべてNG
- [ ] **フィルター使用時も{{ 変数|filter }}形式（スペースなし）**
...
- [ ] 上記以外のmarketplace pluginを使用していない
...
- [ ] parameter-extractorの必須項目（required: true）は最小限に限定されている
- [ ] parameter-extractorの型指定が適切（特にnumber vs string）
...
- [ ] trimやtojsonなどのフィルタが適切に使用されている（スペースなし）
...
##### Google Sheets batch_getレスポンス処理チェック
- [ ] **batch_getの出力構造を理解している**
  - json[0]["valueRanges"][0]["values"]のパス
- [ ] **template-transformでのアクセス時、インデックス[0]を忘れていない**
- [ ] **ブラケット記法（["key"]）を使用している（ドット記法は不可）**
- [ ] **存在チェックまたはdefaultフィルタを使用している**
- [ ] **Jinja2フィルターのパイプ記号後にスペースがない**
  - NG: `{{ row | join(',') }}`
  - OK: `{{ row|join(',') }}`

##### Google Sheetsデータ整形パターンチェック
- [ ] **行データの結合：`{{ row|join(',') }}`を使用**
- [ ] **JSON出力：`{{ data|tojson }}`を使用（jsonではない）**
- [ ] **空データ対策：`|default([])`または`|default('')`を使用**
- [ ] **ループ処理：`{% for row in rows %}`で適切に処理**
- [ ] **template-transformのvariablesでvalue_typeを明示**
  - array[object]：複数行データ
  - string：単一値
...
- [ ] tool_name が "openai_audio_stt" に設定されているか
- [ ] file パラメータが [sys, files] を参照しているか
- [ ] 翻訳機能を使用する場合は whisper-1 モデルを選択しているか
- [ ] ストリーミングを使用する場合は GPT-4o モデルを選択しているか
...
- [ ] vision.configs.detail が "high" または "low" に設定されているか
- [ ] vision.configs.variable_selector が [] に設定されているか
- [ ] 画像ファイルサイズ制限が適切に設定されているか（10MB以下）
- [ ] PDFファイルを処理する場合、allowed_file_types に document が含まれているか

---

## 14. 開発依頼プロンプトへの追加推奨事項
